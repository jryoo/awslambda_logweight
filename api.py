...
"""
need to be able to convert to correct time zones because fitbit does not support 
timezones
"""
...
class FitbitOauth2Client(object):
    ...
    def __init__(self, client_id, client_secret, access_token=None,
        ...
        to start the setup for user authorization
            - client_id, client_secret are in the app configuration apage
    ...
    def _request(self, method, url, **kwargs):
        ...
        try:
            ...
            # If the current token has a no expires_at, or something manages to slip
            # through the crack
            ...
            raise exceptions_helper.Timeout(*e.args)
    ...
    def make_request(self, url, data=None, method=None, **kwargs):
        ...
        exceptions_helper.detect_and_raise_error(response)
...
class Fitbit(object):
    ...
    def make_request(self, *args, **kwargs):
        ...
        if method == 'DELETE':
            if response.status_code == 204:
                ...
                raise exceptions_helper.DeleteError(response)
        try:
            rep = json.loads(response.content.decode('utf8'))
        except ValueError:
            raise exceptions_helper.BadResponse
    ...
    def _convert_timezones(self, date, toZone=to_zone):
    ...
    def set_bodyweight(self, weight, base_date=None, user_id=None, period=None, end_date=None):
        """
        https://dev.fitbit.com/reference/web-api/body/#log-weight
        
        Arguments
        weight: required Weight - in the format X.XX.
        date: required Log entry date - in the format yyyy-MM-dd.
        time: optional Time of the measurement - hours and minutes in the format HH:mm:ss
            which is set to the last second of the day if time is not provided.
        """
        now = self._convert_timezones(datetime.datetime.now())
        data = {
            'weight': weight,
            'date': self._get_date_string(now),
            'time': now.strftime("%H:%M:%S"),
        }
        url = "{0}/{1}/user/{2}/body/log/weight.json".format(*self._get_common_args(user_id))
        return self.make_request(url, data=data, method="POST")
    ...
    def set_bodyfat(self, base_date=None, user_id=None, period=None, end_date=None):
        """
        https://dev.fitbit.com/reference/web-api/body/#log-body-fat
        base_date should be a datetime.date object (defaults to today),
        period can be '1d', '7d', '30d', '1w', '1m', '3m', '6m', '1y', 'max' or None
        end_date should be a datetime.date object, or None.
        You can specify period or end_date, or neither, but not both.
